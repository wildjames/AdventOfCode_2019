...
int parse_wire(char wire[], int wire_size, int wire_flag)
{
    ...
    // Start at the origin
    int position[2] = {board_width/2, board_height/2};
    ...
    while (i<wire_size-1)
    {
        ...
        for (k=0; k < len; k++)
        {
            ...
            switch (board[position[0]][position[1]])
            {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    if (wire_flag == 3) set_to = 4;
                    printf("I crossed a wire at %d %d! Is it my own wire? %d\n", position[0], position[1], (wire_flag == 2));
                    break;
                case 3:
                    if (wire_flag == 2) set_to = 4;
                    printf("I crossed a wire at %d %d! Is it my own wire? %d\n", position[0], position[1], (wire_flag == 3));
                    break;
            }
        ...
        // cout << "Position is now " << position[0] << ", " << position[1] << endl;
    }
    cout << "Done wire!!\n";
    return 0;
}
...
int main(void)
{
    ...
    for (int i=0; i<board_height; i++)
    {
        for (int j=0; j<board_width; j++)
        {
            if (board[j][i] == 4)
            {
                current_dist = abs((j-origin[0])+(i-origin[1]));
                if (DEBUG) cout << "X" << " ";
                printf("Distance found at %d, %d, spaced %d apart\n", j, i, current_dist);
    ...
    cout << "Shortest distance to the wires crossing is " << shortest_dist << endl;
